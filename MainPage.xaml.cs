...
namespace VSProjectTextExport
{
    public partial class MainPage : ContentPage
    {
        ...
        private string ListAllFilesAndDirectories(string spath)
        {
            ...
            var allFileTypes = new string[] { "*.cs", "*.xaml", "*.sln", "*.csproj", "*.resx", "*.json" };
            ...
            var allDirectories = Directory.EnumerateDirectories(rootPath, "*", SearchOption.AllDirectories)
                .Where(
                    dir => !dir.Split(Path.DirectorySeparatorChar)
                        .Any(
                            part => part.StartsWith(".") ||
                                    part.Equals("bin") ||
                                    part.Equals("obj") ||
                                    part.Equals("Platforms") ||
                                    part.Equals("Resources")))
                .OrderBy(dir => dir);
            ...
            foreach(var dir in allDirectories)
            {
                ...
                var allFilesInDir = allFileTypes
                    .SelectMany(ext => Directory.EnumerateFiles(dir, ext, SearchOption.TopDirectoryOnly))
                    .Select(file => Path.GetFileName(file))  // Extract the file names
                    .OrderBy(fileName => fileName);         // Sort the file names
            ...
            var allFilesInRoot = allFileTypes
                .SelectMany(ext => Directory.EnumerateFiles(rootPath, ext, SearchOption.TopDirectoryOnly))
                .Select(file => Path.GetFileName(file))  // Extract the file names
                .OrderBy(fileName => fileName);          // Sort the file names
            ...
            var filesListingString = "Files and Folders in Project " +
                firstRelativeFolder +
                ":" +
                Environment.NewLine +
                string.Join(Environment.NewLine, resultContent) +
                Environment.NewLine +
                Environment.NewLine;
        ...
        private async void OnPathButtonClicked(object sender, EventArgs e)
        {
            try
            {
                var options = new PickOptions
                {
                    PickerTitle = "Please choose a folder"
                };
        ...
        private async void OnPickFolderClicked(object sender, EventArgs e)
        {
            try
            {
                ...
                if(result != null)
                {
                    _selectedPath = Path.GetDirectoryName(result.FullPath);
        ...
        private async void OnCreateReportClicked(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                var filesInDirectories = allDirectories.SelectMany(
                    dir => allFileTypes.SelectMany(
                        ext => Directory.EnumerateFiles(dir, ext, SearchOption.TopDirectoryOnly)))
                    .OrderBy(dir => dir);
                ...
                var csprojFilesInRoot = Directory.EnumerateFiles(
                    _selectedPath,
                    "*.csproj",
                    SearchOption.TopDirectoryOnly);
                var otherFilesInRoot = allFileTypes.Where(ext => ext != "*.sln" && ext != "*.csproj")
                    .SelectMany(ext => Directory.EnumerateFiles(_selectedPath, ext, SearchOption.TopDirectoryOnly))
                    .OrderBy(dir => dir);
                ...
                var resultContent = allFiles.Select(
                    path => new
                    {
            ...
            if(!string.IsNullOrWhiteSpace(_outputFilePath) && File.Exists(_outputFilePath))
            {
                await Launcher.Default
                    .OpenAsync(
                        new OpenFileRequest
                        {
                            File = new ReadOnlyFile(_outputFilePath)
                        });
            }
            else
            {
